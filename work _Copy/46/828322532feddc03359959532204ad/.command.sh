...
# Install compatible versions of numpy and pandas
    ...
    echo "Starting core SNP extraction for cluster cluster_5"
    echo "Input alignment file: cluster_5.filtered_polymorphic_sites.fasta"
    echo "Alignment file size: $(wc -c < "cluster_5.filtered_polymorphic_sites.fasta") bytes"
    echo "Number of sequences: $(grep -c "^>" "cluster_5.filtered_polymorphic_sites.fasta" 2>/dev/null || echo "0")"
    echo ""
...
# Run extraction
extract_core_snps("cluster_5.filtered_polymorphic_sites.fasta", "cluster_5", "clusters.tsv")
EOF

    echo ""
    echo "Core SNP extraction completed for cluster cluster_5"
    echo "Output files:"
    ls -la cluster_5_core_snps.fa cluster_5_snp_positions.tsv
    echo "Core SNPs file size: $(wc -c < "cluster_5_core_snps.fa") bytes"
    echo "SNP positions file size: $(wc -c < "cluster_5_snp_positions.tsv") bytes"
