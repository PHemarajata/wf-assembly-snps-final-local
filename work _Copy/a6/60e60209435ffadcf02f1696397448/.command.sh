...
if [ $seq_count -lt 3 ]; then
    ...
    echo "Skipping tree construction for cluster cluster_2"

    # Create empty output files to satisfy pipeline expectations
    touch cluster_2.treefile
    touch cluster_2.iqtree
...
iqtree2 \
    ...
    --prefix cluster_2 \
     || {
    echo "WARNING: IQ-TREE failed for cluster cluster_2. Creating empty output files."
    touch cluster_2.treefile
    touch cluster_2.iqtree
}

# Ensure all required output files exist (in case IQ-TREE partially failed)
if [ ! -f "cluster_2.treefile" ]; then
    echo "WARNING: Missing treefile for cluster cluster_2. Creating empty file."
    touch cluster_2.treefile
fi

if [ ! -f "cluster_2.iqtree" ]; then
    echo "WARNING: Missing iqtree log for cluster cluster_2. Creating empty file."
    touch cluster_2.iqtree
fi
