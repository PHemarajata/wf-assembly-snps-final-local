...
if [ $seq_count -lt 3 ]; then
    ...
    echo "Skipping tree construction for cluster cluster_1"

    # Create empty output files to satisfy pipeline expectations
    touch cluster_1.treefile
    touch cluster_1.iqtree
...
iqtree2 \
    ...
    --prefix cluster_1 \
     || {
    echo "WARNING: IQ-TREE failed for cluster cluster_1. Creating empty output files."
    touch cluster_1.treefile
    touch cluster_1.iqtree
}

# Ensure all required output files exist (in case IQ-TREE partially failed)
if [ ! -f "cluster_1.treefile" ]; then
    echo "WARNING: Missing treefile for cluster cluster_1. Creating empty file."
    touch cluster_1.treefile
fi

if [ ! -f "cluster_1.iqtree" ]; then
    echo "WARNING: Missing iqtree log for cluster cluster_1. Creating empty file."
    touch cluster_1.iqtree
fi
