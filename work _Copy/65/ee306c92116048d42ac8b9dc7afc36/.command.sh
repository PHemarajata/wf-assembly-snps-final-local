...
if [ $seq_count -lt 3 ]; then
    ...
    echo "Skipping Gubbins analysis for cluster cluster_3"

    # Create empty output files to satisfy pipeline expectations
    touch cluster_3.filtered_polymorphic_sites.fasta
    touch cluster_3.recombination_predictions.gff
    touch cluster_3.node_labelled.final_tree.tre
...
if [ "true" = "true" ]; then
    ...
    run_gubbins.py \
        --starting-tree cluster_3.treefile \
        --prefix cluster_3 \
        ...
        --threads 4 \
         \
        cluster_3.aln.fa || {
        echo "WARNING: Gubbins failed for cluster cluster_3. Creating empty output files."
        touch cluster_3.filtered_polymorphic_sites.fasta
        touch cluster_3.recombination_predictions.gff
        touch cluster_3.node_labelled.final_tree.tre
    }
...
if [ ! -f "cluster_3.filtered_polymorphic_sites.fasta" ]; then
    echo "WARNING: Missing filtered_polymorphic_sites.fasta for cluster cluster_3. Creating empty file."
    touch cluster_3.filtered_polymorphic_sites.fasta
fi

if [ ! -f "cluster_3.recombination_predictions.gff" ]; then
    echo "WARNING: Missing recombination_predictions.gff for cluster cluster_3. Creating empty file."
    touch cluster_3.recombination_predictions.gff
fi

if [ ! -f "cluster_3.node_labelled.final_tree.tre" ]; then
    echo "WARNING: Missing node_labelled.final_tree.tre for cluster cluster_3. Creating empty file."
    touch cluster_3.node_labelled.final_tree.tre
fi
