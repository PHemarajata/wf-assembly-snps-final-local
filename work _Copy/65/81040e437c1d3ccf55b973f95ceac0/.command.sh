...
# Install compatible versions of numpy and pandas
    ...
    echo "Starting core SNP extraction for cluster cluster_4"
    echo "Input alignment file: cluster_4.filtered_polymorphic_sites.fasta"
    echo "Alignment file size: $(wc -c < "cluster_4.filtered_polymorphic_sites.fasta") bytes"
    echo "Number of sequences: $(grep -c "^>" "cluster_4.filtered_polymorphic_sites.fasta" 2>/dev/null || echo "0")"
    echo ""
...
EOF
    ...
    echo ""
    echo "Core SNP extraction completed for cluster cluster_4"
    echo "Output files:"
    ls -la cluster_4_core_snps.fa cluster_4_snp_positions.tsv
    echo "Core SNPs file size: $(wc -c < "cluster_4_core_snps.fa") bytes"
    echo "SNP positions file size: $(wc -c < "cluster_4_snp_positions.tsv") bytes"
