...
mkdir inputfiles
cp IE-0018.fasta inputfiles/"IE-0018.fasta"
...
echo -e "Sample name	QC step	Outcome (Pass/Fail)" > "IE-0018.Initial_Input_File.tsv"
for file in inputfiles/*; do
  if verify_minimum_file_size "${file}" 'Input' "1k"; then
    echo -e "$(basename ${file%%.*})	Input File	PASS"         >> "IE-0018.Initial_Input_File.tsv"
  else
    echo -e "$(basename ${file%%.*})	Input File	FAIL"         >> "IE-0018.Initial_Input_File.tsv"
