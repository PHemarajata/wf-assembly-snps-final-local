...
seq_count=$(grep -c "^>" cluster_2.aln.fa)
...
if [ $seq_count -lt 3 ]; then
    ...
    echo "Skipping Gubbins analysis for cluster cluster_2"
...
if [ "true" = "true" ]; then
    ...
    run_gubbins.py \
        ...
        --threads 4 \
         \
        cluster_2.aln.fa || {
        echo "WARNING: Gubbins failed for cluster cluster_2. Creating empty output files."
        touch cluster_2.filtered_polymorphic_sites.fasta
        touch cluster_2.recombination_predictions.gff
        touch cluster_2.node_labelled.final_tree.tre
    }
...
if [ ! -f "cluster_2.filtered_polymorphic_sites.fasta" ]; then
    echo "WARNING: Missing filtered_polymorphic_sites.fasta for cluster cluster_2. Creating empty file."
    touch cluster_2.filtered_polymorphic_sites.fasta
fi

if [ ! -f "cluster_2.recombination_predictions.gff" ]; then
    echo "WARNING: Missing recombination_predictions.gff for cluster cluster_2. Creating empty file."
    touch cluster_2.recombination_predictions.gff
fi

if [ ! -f "cluster_2.node_labelled.final_tree.tre" ]; then
    echo "WARNING: Missing node_labelled.final_tree.tre for cluster cluster_2. Creating empty file."
    touch cluster_2.node_labelled.final_tree.tre
fi
