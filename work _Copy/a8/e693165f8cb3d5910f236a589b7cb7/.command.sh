...
if [ $seq_count -lt 3 ]; then
    ...
    echo "Skipping tree construction for cluster cluster_4"

    # Create empty output files to satisfy pipeline expectations
    touch cluster_4.treefile
    touch cluster_4.iqtree
...
iqtree2 \
    ...
    --prefix cluster_4 \
     || {
    echo "WARNING: IQ-TREE failed for cluster cluster_4. Creating empty output files."
    touch cluster_4.treefile
    touch cluster_4.iqtree
}

# Ensure all required output files exist (in case IQ-TREE partially failed)
if [ ! -f "cluster_4.treefile" ]; then
    echo "WARNING: Missing treefile for cluster cluster_4. Creating empty file."
    touch cluster_4.treefile
fi

if [ ! -f "cluster_4.iqtree" ]; then
    echo "WARNING: Missing iqtree log for cluster cluster_4. Creating empty file."
    touch cluster_4.iqtree
fi
