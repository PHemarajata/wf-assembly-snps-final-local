...
if [ $seq_count -lt 3 ]; then
    ...
    echo "Skipping tree construction for cluster cluster_6"

    # Create empty output files to satisfy pipeline expectations
    touch cluster_6.treefile
    touch cluster_6.iqtree
...
iqtree2 \
    ...
    --prefix cluster_6 \
     || {
    echo "WARNING: IQ-TREE failed for cluster cluster_6. Creating empty output files."
    touch cluster_6.treefile
    touch cluster_6.iqtree
}

# Ensure all required output files exist (in case IQ-TREE partially failed)
if [ ! -f "cluster_6.treefile" ]; then
    echo "WARNING: Missing treefile for cluster cluster_6. Creating empty file."
    touch cluster_6.treefile
fi

if [ ! -f "cluster_6.iqtree" ]; then
    echo "WARNING: Missing iqtree log for cluster cluster_6. Creating empty file."
    touch cluster_6.iqtree
fi
