...
mkdir inputfiles
cp IP-0117.fasta inputfiles/"IP-0117.fasta"
...
echo -e "Sample name	QC step	Outcome (Pass/Fail)" > "IP-0117.Initial_Input_File.tsv"
for file in inputfiles/*; do
  if verify_minimum_file_size "${file}" 'Input' "1k"; then
    echo -e "$(basename ${file%%.*})	Input File	PASS"         >> "IP-0117.Initial_Input_File.tsv"
  else
    echo -e "$(basename ${file%%.*})	Input File	FAIL"         >> "IP-0117.Initial_Input_File.tsv"
