...
mkdir inputfiles
cp ERS013346.fasta inputfiles/"ERS013346.fasta"
...
echo -e "Sample name	QC step	Outcome (Pass/Fail)" > "ERS013346.Initial_Input_File.tsv"
for file in inputfiles/*; do
  if verify_minimum_file_size "${file}" 'Input' "1k"; then
    echo -e "$(basename ${file%%.*})	Input File	PASS"         >> "ERS013346.Initial_Input_File.tsv"
  else
    echo -e "$(basename ${file%%.*})	Input File	FAIL"         >> "ERS013346.Initial_Input_File.tsv"
