...
# Install compatible versions of numpy and pandas
    ...
    echo "Starting core SNP extraction for cluster cluster_2"
    echo "Input alignment file: cluster_2.filtered_polymorphic_sites.fasta"
    echo "Alignment file size: $(wc -c < "cluster_2.filtered_polymorphic_sites.fasta") bytes"
    echo "Number of sequences: $(grep -c "^>" "cluster_2.filtered_polymorphic_sites.fasta" 2>/dev/null || echo "0")"
    echo ""
...
# Run extraction
extract_core_snps("cluster_2.filtered_polymorphic_sites.fasta", "cluster_2", "clusters.tsv")
EOF

    echo ""
    echo "Core SNP extraction completed for cluster cluster_2"
    echo "Output files:"
    ls -la cluster_2_core_snps.fa cluster_2_snp_positions.tsv
    echo "Core SNPs file size: $(wc -c < "cluster_2_core_snps.fa") bytes"
    echo "SNP positions file size: $(wc -c < "cluster_2_snp_positions.tsv") bytes"
