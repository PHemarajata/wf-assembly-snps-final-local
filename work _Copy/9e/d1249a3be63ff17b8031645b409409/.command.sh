...
echo "Checking integrated alignment file: integrated_core_snps.fa"
    
    # Check if alignment file exists and has content
    if [ ! -f "integrated_core_snps.fa" ]; then
        echo "ERROR: Alignment file integrated_core_snps.fa does not exist"
        seq_count=0
    elif [ ! -s "integrated_core_snps.fa" ]; then
        echo "WARNING: Alignment file integrated_core_snps.fa is empty"
        seq_count=0
    else
        # Count sequences safely
        seq_count=$(grep -c "^>" "integrated_core_snps.fa" 2>/dev/null || echo "0")
    ...
    if [ "$seq_count" -lt 3 ]; then
        ...
        echo "Alignment file size: $(wc -c < "integrated_core_snps.fa") bytes"
        echo "First few lines of alignment:"
        head -10 "integrated_core_snps.fa" || echo "Could not read alignment file"
        
        # Build tree with IQ-TREE
        iqtree2 \
            -s integrated_core_snps.fa \
            -st DNA \
            -m MFP \
            -bb 1000 \
            -alrt 1000 \
...
try:
    ...
    mapping_data = []
    try:
        with open("sample_cluster_mapping.tsv", 'r') as f:
            lines = f.readlines()
...
EOF
    ...
    "ASSEMBLY_SNPS_SCALABLE:INTEGRATE_RESULTS:BUILD_INTEGRATED_TREE":
