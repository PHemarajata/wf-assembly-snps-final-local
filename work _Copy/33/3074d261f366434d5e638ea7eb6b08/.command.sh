...
mkdir inputfiles
cp ERS012330.fasta inputfiles/"ERS012330.fasta"
...
echo -e "Sample name	QC step	Outcome (Pass/Fail)" > "ERS012330.Initial_Input_File.tsv"
for file in inputfiles/*; do
  if verify_minimum_file_size "${file}" 'Input' "1k"; then
    echo -e "$(basename ${file%%.*})	Input File	PASS"         >> "ERS012330.Initial_Input_File.tsv"
  else
    echo -e "$(basename ${file%%.*})	Input File	FAIL"         >> "ERS012330.Initial_Input_File.tsv"
