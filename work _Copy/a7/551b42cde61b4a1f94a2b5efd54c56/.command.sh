...
# Install compatible versions of numpy and pandas
    ...
    echo "Starting core SNP extraction for cluster cluster_3"
    echo "Input alignment file: cluster_3.filtered_polymorphic_sites.fasta"
    echo "Alignment file size: $(wc -c < "cluster_3.filtered_polymorphic_sites.fasta") bytes"
    echo "Number of sequences: $(grep -c "^>" "cluster_3.filtered_polymorphic_sites.fasta" 2>/dev/null || echo "0")"
    echo ""
...
# Run extraction
extract_core_snps("cluster_3.filtered_polymorphic_sites.fasta", "cluster_3", "clusters.tsv")
EOF

    echo ""
    echo "Core SNP extraction completed for cluster cluster_3"
    echo "Output files:"
    ls -la cluster_3_core_snps.fa cluster_3_snp_positions.tsv
    echo "Core SNPs file size: $(wc -c < "cluster_3_core_snps.fa") bytes"
    echo "SNP positions file size: $(wc -c < "cluster_3_snp_positions.tsv") bytes"
