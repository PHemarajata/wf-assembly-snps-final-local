...
if [ $seq_count -lt 3 ]; then
    ...
    echo "Skipping tree construction for cluster cluster_5"

    # Create empty output files to satisfy pipeline expectations
    touch cluster_5.treefile
    touch cluster_5.iqtree
...
iqtree2 \
    ...
    --prefix cluster_5 \
     || {
    echo "WARNING: IQ-TREE failed for cluster cluster_5. Creating empty output files."
    touch cluster_5.treefile
    touch cluster_5.iqtree
}

# Ensure all required output files exist (in case IQ-TREE partially failed)
if [ ! -f "cluster_5.treefile" ]; then
    echo "WARNING: Missing treefile for cluster cluster_5. Creating empty file."
    touch cluster_5.treefile
fi

if [ ! -f "cluster_5.iqtree" ]; then
    echo "WARNING: Missing iqtree log for cluster cluster_5. Creating empty file."
    touch cluster_5.iqtree
fi
