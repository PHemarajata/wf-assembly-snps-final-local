...
# Install compatible versions of numpy and pandas
    ...
    echo "Starting core SNP extraction for cluster cluster_6"
    echo "Input alignment file: cluster_6.filtered_polymorphic_sites.fasta"
    echo "Alignment file size: $(wc -c < "cluster_6.filtered_polymorphic_sites.fasta") bytes"
    echo "Number of sequences: $(grep -c "^>" "cluster_6.filtered_polymorphic_sites.fasta" 2>/dev/null || echo "0")"
    echo ""
...
# Run extraction
extract_core_snps("cluster_6.filtered_polymorphic_sites.fasta", "cluster_6", "clusters.tsv")
EOF

    echo ""
    echo "Core SNP extraction completed for cluster cluster_6"
    echo "Output files:"
    ls -la cluster_6_core_snps.fa cluster_6_snp_positions.tsv
    echo "Core SNPs file size: $(wc -c < "cluster_6_core_snps.fa") bytes"
    echo "SNP positions file size: $(wc -c < "cluster_6_snp_positions.tsv") bytes"
