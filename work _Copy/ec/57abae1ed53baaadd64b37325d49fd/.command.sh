...
mkdir inputfiles
cp ERS013348.fasta inputfiles/"ERS013348.fasta"
...
echo -e "Sample name	QC step	Outcome (Pass/Fail)" > "ERS013348.Initial_Input_File.tsv"
for file in inputfiles/*; do
  if verify_minimum_file_size "${file}" 'Input' "1k"; then
    echo -e "$(basename ${file%%.*})	Input File	PASS"         >> "ERS013348.Initial_Input_File.tsv"
  else
    echo -e "$(basename ${file%%.*})	Input File	FAIL"         >> "ERS013348.Initial_Input_File.tsv"
